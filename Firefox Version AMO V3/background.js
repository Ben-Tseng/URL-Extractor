...
const EXTRACT_SHORTCUT_COMMAND = "extract-urls-shortcut";
...
api.contextMenus.onClicked.addListener(async (info, tab) => {
  ...
  await runExtractionFlow(info.selectionText || "", tab);
});

api.commands.onCommand.addListener(async (command) => {
  if (command !== EXTRACT_SHORTCUT_COMMAND) {
    return;
  }
  ...
  await runExtractionFlow("", activeTab);
});

async function runExtractionFlow(initialSelectionText, tab) {
  const tabId = await resolveTabId(tab);

  let selectionText = initialSelectionText || "";
